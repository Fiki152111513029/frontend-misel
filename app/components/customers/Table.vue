...
export type CustomerSortKey = CustomerSortBy | 'isActive'
...
const emit = defineEmits<{
  ...
  sort: [patch: { sortBy: CustomerSortKey, sortOrder: CustomerSortOrder }]
}>()
...
const activeSort = ref<{ key: CustomerSortKey, order: CustomerSortOrder }>({
  key: props.sortBy ?? 'name',
  order: props.sortOrder ?? 'asc',
})

function toggleSort(key: CustomerSortKey) {
  const order: CustomerSortOrder = activeSort.value.key === key && activeSort.value.order === 'asc' ? 'desc' : 'asc'
      ...
      <template #header>
        ...
        >
          <button
            ...
            @click="toggleSort(col.key as CustomerSortKey)"
          >
            {{ col.label }}
