...
import type {
  ChargerArea,
  ChargerAreaSortBy,
  ChargerAreaSortOrder,
} from '~/types/charger-area'

export type ChargerAreaSortKey = ChargerAreaSortBy | 'iRaypleLocationCode' | 'isActive'

interface Props {
  items: ChargerArea[]
  loading: boolean
  sortBy?: ChargerAreaSortKey
...
const emit = defineEmits<{
  edit: [chargerArea: ChargerArea]
  delete: [chargerArea: ChargerArea]
  sort: [patch: { sortBy: ChargerAreaSortKey, sortOrder: ChargerAreaSortOrder }]
}>()
...
const activeSort = ref<{ key: ChargerAreaSortKey, order: ChargerAreaSortOrder }>({
  key: props.sortBy ?? 'name',
  order: props.sortOrder ?? 'asc',
})

function toggleSort(key: ChargerAreaSortKey) {
  const order: ChargerAreaSortOrder = activeSort.value.key === key && activeSort.value.order === 'asc' ? 'desc' : 'asc'
      ...
      <template #header>
        ...
        >
          <button
            ...
            @click="toggleSort(col.key as ChargerAreaSortKey)"
          >
            {{ col.label }}
 ...
 : 'bg-slate-100 text-slate-500 '"
          ...
          <td class="px-4 py-3">
            <div class="flex items-center gap-2">
              <button
                v-if="hasPermission('charger-area.update')"
                type="button"
              ...
              <button
                v-if="hasPermission('charger-area.delete')"
                type="button"
