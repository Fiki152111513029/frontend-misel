...
import type {
  TrolleyCategory,
  TrolleyCategorySortBy,
  TrolleyCategorySortOrder,
} from '~/types/trolley-category'

export type TrolleyCategorySortKey = TrolleyCategorySortBy

interface Props {
  items: TrolleyCategory[]
  loading: boolean
  sortBy?: TrolleyCategorySortKey
...
const emit = defineEmits<{
  edit: [trolleyCategory: TrolleyCategory]
  delete: [trolleyCategory: TrolleyCategory]
  sort: [patch: { sortBy: TrolleyCategorySortKey, sortOrder: TrolleyCategorySortOrder }]
}>()
...
const activeSort = ref<{ key: TrolleyCategorySortKey, order: TrolleyCategorySortOrder }>({
  key: props.sortBy ?? 'name',
  order: props.sortOrder ?? 'asc',
})

function toggleSort(key: TrolleyCategorySortKey) {
  const order: TrolleyCategorySortOrder = activeSort.value.key === key && activeSort.value.order === 'asc' ? 'desc' : 'asc'
      ...
      <template #header>
        ...
        >
          <button
            ...
            @click="toggleSort(col.key as TrolleyCategorySortKey)"
          >
            {{ col.label }}
        ...
        >
          ...
          <td class="px-4 py-3">
            <div class="flex items-center gap-2">
              <button
                v-if="hasPermission('trolley-category.update')"
                type="button"
              ...
              <button
                v-if="hasPermission('trolley-category.delete')"
                type="button"
