...
import type {
  TrolleyType,
  TrolleyTypeSortBy,
  TrolleyTypeSortOrder,
} from '~/types/trolley-type'

export type TrolleyTypeSortKey = TrolleyTypeSortBy | 'isActive'

interface Props {
  items: TrolleyType[]
  loading: boolean
  sortBy?: TrolleyTypeSortKey
...
const emit = defineEmits<{
  edit: [trolleyType: TrolleyType]
  delete: [trolleyType: TrolleyType]
  sort: [patch: { sortBy: TrolleyTypeSortKey, sortOrder: TrolleyTypeSortOrder }]
}>()
...
const activeSort = ref<{ key: TrolleyTypeSortKey, order: TrolleyTypeSortOrder }>({
  key: props.sortBy ?? 'name',
  order: props.sortOrder ?? 'asc',
})

function toggleSort(key: TrolleyTypeSortKey) {
  const order: TrolleyTypeSortOrder = activeSort.value.key === key && activeSort.value.order === 'asc' ? 'desc' : 'asc'
      ...
      <template #header>
        ...
        >
          <button
            ...
            @click="toggleSort(col.key as TrolleyTypeSortKey)"
          >
            {{ col.label }}
 ...
 : 'bg-slate-100 text-slate-500 '"
          ...
          <td class="px-4 py-3">
            <div class="flex items-center gap-2">
              <button
                v-if="hasPermission('trolley-type.update')"
                type="button"
              ...
              <button
                v-if="hasPermission('trolley-type.delete')"
                type="button"
