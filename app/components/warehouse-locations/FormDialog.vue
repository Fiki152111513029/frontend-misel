...
const emit = defineEmits<{
  ...
  submit: [input: CreateWarehouseLocationInput]
  cancel: []
}>()
...
function resetFields() {
  name.value = props.warehouseLocation?.name ?? ''
  iRaypleLocationCode.value = props.warehouseLocation?.iRaypleLocationCode ?? ''
  isActive.value = props.warehouseLocation?.isActive ?? true
...
const isEditMode = computed(() => !!props.warehouseLocation)
...
<template>
  <UiBaseModal
    ...
    :title="isEditMode ? 'Edit Warehouse Location' : 'Add Warehouse Location'"
    size="sm"
