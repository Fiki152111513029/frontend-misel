...
import type {
  ParkingArea,
  ParkingAreaSortBy,
  ParkingAreaSortOrder,
} from '~/types/parking-area'

export type ParkingAreaSortKey = ParkingAreaSortBy | 'iRaypleLocationCode' | 'isActive'

interface Props {
  items: ParkingArea[]
  loading: boolean
  sortBy?: ParkingAreaSortKey
...
const emit = defineEmits<{
  edit: [parkingArea: ParkingArea]
  delete: [parkingArea: ParkingArea]
  sort: [patch: { sortBy: ParkingAreaSortKey, sortOrder: ParkingAreaSortOrder }]
}>()
...
const activeSort = ref<{ key: ParkingAreaSortKey, order: ParkingAreaSortOrder }>({
  key: props.sortBy ?? 'name',
  order: props.sortOrder ?? 'asc',
})

function toggleSort(key: ParkingAreaSortKey) {
  const order: ParkingAreaSortOrder = activeSort.value.key === key && activeSort.value.order === 'asc' ? 'desc' : 'asc'
      ...
      <template #header>
        ...
        >
          <button
            ...
            @click="toggleSort(col.key as ParkingAreaSortKey)"
          >
            {{ col.label }}
 ...
 : 'bg-slate-100 text-slate-500 '"
          ...
          <td class="px-4 py-3">
            <div class="flex items-center gap-2">
              <button
                v-if="hasPermission('parking-area.update')"
                type="button"
              ...
              <button
                v-if="hasPermission('parking-area.delete')"
                type="button"
