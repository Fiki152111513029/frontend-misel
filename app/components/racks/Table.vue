...
import type { Rack, RackSortBy, RackSortOrder } from '~/types/rack'

export type RackSortKey = RackSortBy | 'status' | 'isActive'
...
const emit = defineEmits<{
  edit: [rack: Rack]
  delete: [rack: Rack]
  sort: [patch: { sortBy: RackSortKey, sortOrder: RackSortOrder }]
}>()
...
const activeSort = ref<{ key: RackSortKey, order: RackSortOrder }>({
  key: props.sortBy ?? 'name',
  order: props.sortOrder ?? 'asc',
})

function toggleSort(key: RackSortKey) {
  const order: RackSortOrder = activeSort.value.key === key && activeSort.value.order === 'asc' ? 'desc' : 'asc'
      ...
      <template #header>
        ...
        >
          <button
            ...
            @click="toggleSort(col.key as RackSortKey)"
          >
            {{ col.label }}
        ...
        >
          ...
          <td class="px-4 py-3">
            <div class="flex items-center gap-2">
              <button
                v-if="hasPermission('rack.update')"
                type="button"
              ...
              <button
                v-if="hasPermission('rack.delete')"
                type="button"
