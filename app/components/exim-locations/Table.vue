...
import type { EximLocation, EximLocationSortBy, EximLocationSortOrder } from '~/types/exim-location'

export type EximLocationSortKey = EximLocationSortBy | 'iRaypleLocationCode' | 'isActive'

interface Props {
  items: EximLocation[]
  loading: boolean
  sortBy?: EximLocationSortKey
...
const emit = defineEmits<{
  edit: [eximLocation: EximLocation]
  delete: [eximLocation: EximLocation]
  sort: [patch: { sortBy: EximLocationSortKey, sortOrder: EximLocationSortOrder }]
}>()
...
const activeSort = ref<{ key: EximLocationSortKey, order: EximLocationSortOrder }>({
  key: props.sortBy ?? 'name',
  order: props.sortOrder ?? 'asc',
})

function toggleSort(key: EximLocationSortKey) {
  const order: EximLocationSortOrder = activeSort.value.key === key && activeSort.value.order === 'asc' ? 'desc' : 'asc'
      ...
      <template #header>
        ...
        >
          <button
            ...
            @click="toggleSort(col.key as EximLocationSortKey)"
          >
            {{ col.label }}
 ...
 : 'bg-slate-100 text-slate-500 '"
          ...
          <td class="px-4 py-3">
            <div class="flex items-center gap-2">
              <button
                v-if="hasPermission('exim-location.update')"
                type="button"
              ...
              <button
                v-if="hasPermission('exim-location.delete')"
                type="button"
