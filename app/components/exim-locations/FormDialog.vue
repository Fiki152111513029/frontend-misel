...
interface Props {
  modelValue: boolean
  eximLocation?: EximLocation | null
  submitting?: boolean
}
...
const emit = defineEmits<{
  ...
  submit: [input: CreateEximLocationInput]
  cancel: []
}>()
...
function resetFields() {
  name.value = props.eximLocation?.name ?? ''
  iRaypleLocationCode.value = props.eximLocation?.iRaypleLocationCode ?? ''
  isActive.value = props.eximLocation?.isActive ?? true
...
const isEditMode = computed(() => !!props.eximLocation)
...
<template>
  <UiBaseModal
    ...
    :title="isEditMode ? 'Edit EXIM Location' : 'Add EXIM Location'"
    size="sm"
